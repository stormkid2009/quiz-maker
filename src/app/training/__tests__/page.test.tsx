...
describe("QuestionsPage", () => {
  ...
  it("renders all question type cards with correct titles and descriptions", () => {
    ...
    expect(
      screen.getByText(/multi choices and only one answer/i)
    ).toBeInTheDocument();
    expect(screen.getByText(/situation \(multi-mcq\)/i)).toBeInTheDocument();
    expect(
      screen.getByText(/multi choices with two valid answers/i)
    ).toBeInTheDocument();
    expect(screen.getByText(/composition \(open-ended\)/i)).toBeInTheDocument();
    expect(
      screen.getByText(/open end so the evaluation will be from an expert/i)
    ).toBeInTheDocument();
    expect(screen.getByText(/reading comprehension/i)).toBeInTheDocument();
    expect(
      screen.getByText(/one text for comprehension and multi sub questions/i)
    ).toBeInTheDocument();
  });

  it("renders navigation links for each question type", () => {
    render(<QuestionsPage />);
    const links = screen.getAllByRole("link", { name: /jump to training/i });
  ...
  it("renders the 'Take Full Quiz' button", () => {
    render(<QuestionsPage />);
    expect(
      screen.getByRole("link", { name: /take full quiz/i })
    ).toBeInTheDocument();
  });
});
