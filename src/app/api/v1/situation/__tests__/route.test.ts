...
describe("GET /api/v1/situation", () => {
  const createRequest = (method = "GET") => ({
    method,
    nextUrl: { pathname: "/api/v1/situation" },
  ...
  it("returns a situation question on success", async () => {
    const question = { id: 1, question: "Sample situation question...", options: ["A", "B"], correctAnswers: ["A"] };
  ...
  it("returns 500 on unexpected error", async () => {
    require("@/utils/database").testDatabaseConnection.mockImplementation(() => { throw new Error("fail"); });
    const req = createRequest();
    const res = await GET(req);
    const data = await res.json();
    expect(res.status).toBe(500);
